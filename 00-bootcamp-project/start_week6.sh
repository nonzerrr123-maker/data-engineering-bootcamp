...
if ! grep -q '^GEMINI_API_KEY=.' .env; then
  ...
  echo "  bash 00-bootcamp-project/start_week6.sh"
  exit 2
fi
...
echo 'Checking Week 6 packages inside the Airflow image...'
docker compose exec -T airflow-apiserver   python -c "from google import genai; import pandas_gbq, pyarrow; print('Week 6 packages OK')"

echo 'Waiting for llm_course_data_pipeline to register...'
dag_ok=0
for _ in $(seq 1 90); do
  output="$(docker compose exec -T airflow-scheduler airflow dags list 2>&1 || true)"
  if grep -q 'llm_course_data_pipeline' <<<"$output"; then
    dag_ok=1
    break
  fi
  sleep 2
...
if [[ "$dag_ok" -ne 1 ]]; then
  echo 'ERROR: llm_course_data_pipeline was not registered.'
...
result = client.models.embed_content(
    ...
    contents="Week 6 workshop preflight",
)
...
echo 'Triggering the Week 6 RAG pipeline...'
trigger_output="$(docker compose exec -T airflow-scheduler airflow dags trigger llm_course_data_pipeline 2>&1)"
echo "$trigger_output"

echo 'Waiting for the triggered DAG run to finish...'
run_state=""
for _ in $(seq 1 180); do
  runs="$(docker compose exec -T airflow-scheduler airflow dags list-runs --dag-id llm_course_data_pipeline --output json 2>/dev/null || true)"
...
try:
  ...
  case "$run_state" in
    success)
      ...
      ;;
    failed)
      echo 'ERROR: Week 6 DAG failed.'
      docker compose exec -T airflow-scheduler airflow dags list-runs --dag-id llm_course_data_pipeline || true
      exit 1
      ;;
  esac
  sleep 2
...
if [[ "$run_state" != "success" ]]; then
  echo "ERROR: Timed out waiting for llm_course_data_pipeline (last state: ${run_state:-unknown})."
  exit 1
fi

echo 'Verifying course_embeddings in BigQuery...'
...
echo
echo 'WEEK 6 WORKSHOP RUNTIME PASSED ✅'
echo 'Airflow DAG: llm_course_data_pipeline'
echo 'BigQuery table: project-b9bafacf-46f9-43ef-bcc.deb_bootcamp.course_embeddings'
