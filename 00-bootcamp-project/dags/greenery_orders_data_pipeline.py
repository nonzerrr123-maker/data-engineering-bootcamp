...
def _extract_data(ds):
    ...
    if data:
        with open(f"{DAGS_FOLDER}/{DATA}-{ds}.csv", "w", newline="") as f:
            writer = csv.writer(f)
            header = [
                "order_id", "user_id", "promo_id", "address_id", "created_at",
                "order_cost", "shipping_cost", "order_total", "tracking_id",
                "shipping_service", "estimated_delivery_at", "delivered_at", "status",
            ]
...
default_args = {"owner": "airflow", "start_date": timezone.datetime(2021, 2, 10)}
